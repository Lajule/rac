...
route_node_t *
create_route_node (const char *segment, int is_wildcard)
{
    ...
    while (token && current)
        {
            ...
            for (int i = 0; i < current->child_count; i++)
                {
                    if (current->children[i]->is_wildcard
                        || strcmp (current->children[i]->segment, token) == 0)
                        {
                            found = current->children[i];
                            break;
                        }
...
handle_user_by_id (http_request_t *req, http_response_t *res)
{
    ...
    PGresult *result
        = PQexecParams (conn, "SELECT id, name, email FROM users WHERE id = $1",
...
handle_list_users (http_request_t *req, http_response_t *res)
{
    ...
    PGresult *result
        = PQexec (conn, "SELECT id, name, email FROM users LIMIT 10");
            ...
            if (name && email)
                {
                    ...
                    const char *params[2]
                        = { name->valuestring, email->valuestring };
                    PGresult *result
                        = PQexecParams (conn,
...
main ()
{
    ...
    const char *db_conninfo
        = "host=localhost dbname=mydb user=postgres password=postgres";
